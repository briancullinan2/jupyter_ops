// Generated from Resources/Parsers/oberon/oberon.g4 by ANTLR 4.10.1
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by oberonParser.
export default class oberonListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by oberonParser#ident.
	enterIdent(ctx) {
	}

	// Exit a parse tree produced by oberonParser#ident.
	exitIdent(ctx) {
	}


	// Enter a parse tree produced by oberonParser#qualident.
	enterQualident(ctx) {
	}

	// Exit a parse tree produced by oberonParser#qualident.
	exitQualident(ctx) {
	}


	// Enter a parse tree produced by oberonParser#identdef.
	enterIdentdef(ctx) {
	}

	// Exit a parse tree produced by oberonParser#identdef.
	exitIdentdef(ctx) {
	}


	// Enter a parse tree produced by oberonParser#integer.
	enterInteger(ctx) {
	}

	// Exit a parse tree produced by oberonParser#integer.
	exitInteger(ctx) {
	}


	// Enter a parse tree produced by oberonParser#real.
	enterReal(ctx) {
	}

	// Exit a parse tree produced by oberonParser#real.
	exitReal(ctx) {
	}


	// Enter a parse tree produced by oberonParser#scaleFactor.
	enterScaleFactor(ctx) {
	}

	// Exit a parse tree produced by oberonParser#scaleFactor.
	exitScaleFactor(ctx) {
	}


	// Enter a parse tree produced by oberonParser#number.
	enterNumber(ctx) {
	}

	// Exit a parse tree produced by oberonParser#number.
	exitNumber(ctx) {
	}


	// Enter a parse tree produced by oberonParser#constDeclaration.
	enterConstDeclaration(ctx) {
	}

	// Exit a parse tree produced by oberonParser#constDeclaration.
	exitConstDeclaration(ctx) {
	}


	// Enter a parse tree produced by oberonParser#constExpression.
	enterConstExpression(ctx) {
	}

	// Exit a parse tree produced by oberonParser#constExpression.
	exitConstExpression(ctx) {
	}


	// Enter a parse tree produced by oberonParser#typeDeclaration.
	enterTypeDeclaration(ctx) {
	}

	// Exit a parse tree produced by oberonParser#typeDeclaration.
	exitTypeDeclaration(ctx) {
	}


	// Enter a parse tree produced by oberonParser#type_.
	enterType_(ctx) {
	}

	// Exit a parse tree produced by oberonParser#type_.
	exitType_(ctx) {
	}


	// Enter a parse tree produced by oberonParser#arrayType.
	enterArrayType(ctx) {
	}

	// Exit a parse tree produced by oberonParser#arrayType.
	exitArrayType(ctx) {
	}


	// Enter a parse tree produced by oberonParser#length.
	enterLength(ctx) {
	}

	// Exit a parse tree produced by oberonParser#length.
	exitLength(ctx) {
	}


	// Enter a parse tree produced by oberonParser#recordType.
	enterRecordType(ctx) {
	}

	// Exit a parse tree produced by oberonParser#recordType.
	exitRecordType(ctx) {
	}


	// Enter a parse tree produced by oberonParser#baseType.
	enterBaseType(ctx) {
	}

	// Exit a parse tree produced by oberonParser#baseType.
	exitBaseType(ctx) {
	}


	// Enter a parse tree produced by oberonParser#fieldListSequence.
	enterFieldListSequence(ctx) {
	}

	// Exit a parse tree produced by oberonParser#fieldListSequence.
	exitFieldListSequence(ctx) {
	}


	// Enter a parse tree produced by oberonParser#fieldList.
	enterFieldList(ctx) {
	}

	// Exit a parse tree produced by oberonParser#fieldList.
	exitFieldList(ctx) {
	}


	// Enter a parse tree produced by oberonParser#identList.
	enterIdentList(ctx) {
	}

	// Exit a parse tree produced by oberonParser#identList.
	exitIdentList(ctx) {
	}


	// Enter a parse tree produced by oberonParser#pointerType.
	enterPointerType(ctx) {
	}

	// Exit a parse tree produced by oberonParser#pointerType.
	exitPointerType(ctx) {
	}


	// Enter a parse tree produced by oberonParser#procedureType.
	enterProcedureType(ctx) {
	}

	// Exit a parse tree produced by oberonParser#procedureType.
	exitProcedureType(ctx) {
	}


	// Enter a parse tree produced by oberonParser#variableDeclaration.
	enterVariableDeclaration(ctx) {
	}

	// Exit a parse tree produced by oberonParser#variableDeclaration.
	exitVariableDeclaration(ctx) {
	}


	// Enter a parse tree produced by oberonParser#expression.
	enterExpression(ctx) {
	}

	// Exit a parse tree produced by oberonParser#expression.
	exitExpression(ctx) {
	}


	// Enter a parse tree produced by oberonParser#relation.
	enterRelation(ctx) {
	}

	// Exit a parse tree produced by oberonParser#relation.
	exitRelation(ctx) {
	}


	// Enter a parse tree produced by oberonParser#simpleExpression.
	enterSimpleExpression(ctx) {
	}

	// Exit a parse tree produced by oberonParser#simpleExpression.
	exitSimpleExpression(ctx) {
	}


	// Enter a parse tree produced by oberonParser#addOperator.
	enterAddOperator(ctx) {
	}

	// Exit a parse tree produced by oberonParser#addOperator.
	exitAddOperator(ctx) {
	}


	// Enter a parse tree produced by oberonParser#term.
	enterTerm(ctx) {
	}

	// Exit a parse tree produced by oberonParser#term.
	exitTerm(ctx) {
	}


	// Enter a parse tree produced by oberonParser#mulOperator.
	enterMulOperator(ctx) {
	}

	// Exit a parse tree produced by oberonParser#mulOperator.
	exitMulOperator(ctx) {
	}


	// Enter a parse tree produced by oberonParser#factor.
	enterFactor(ctx) {
	}

	// Exit a parse tree produced by oberonParser#factor.
	exitFactor(ctx) {
	}


	// Enter a parse tree produced by oberonParser#designator.
	enterDesignator(ctx) {
	}

	// Exit a parse tree produced by oberonParser#designator.
	exitDesignator(ctx) {
	}


	// Enter a parse tree produced by oberonParser#selector.
	enterSelector(ctx) {
	}

	// Exit a parse tree produced by oberonParser#selector.
	exitSelector(ctx) {
	}


	// Enter a parse tree produced by oberonParser#set_.
	enterSet_(ctx) {
	}

	// Exit a parse tree produced by oberonParser#set_.
	exitSet_(ctx) {
	}


	// Enter a parse tree produced by oberonParser#element.
	enterElement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#element.
	exitElement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#expList.
	enterExpList(ctx) {
	}

	// Exit a parse tree produced by oberonParser#expList.
	exitExpList(ctx) {
	}


	// Enter a parse tree produced by oberonParser#actualParameters.
	enterActualParameters(ctx) {
	}

	// Exit a parse tree produced by oberonParser#actualParameters.
	exitActualParameters(ctx) {
	}


	// Enter a parse tree produced by oberonParser#statement.
	enterStatement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#statement.
	exitStatement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#assignment.
	enterAssignment(ctx) {
	}

	// Exit a parse tree produced by oberonParser#assignment.
	exitAssignment(ctx) {
	}


	// Enter a parse tree produced by oberonParser#procedureCall.
	enterProcedureCall(ctx) {
	}

	// Exit a parse tree produced by oberonParser#procedureCall.
	exitProcedureCall(ctx) {
	}


	// Enter a parse tree produced by oberonParser#statementSequence.
	enterStatementSequence(ctx) {
	}

	// Exit a parse tree produced by oberonParser#statementSequence.
	exitStatementSequence(ctx) {
	}


	// Enter a parse tree produced by oberonParser#ifStatement.
	enterIfStatement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#ifStatement.
	exitIfStatement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#caseStatement.
	enterCaseStatement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#caseStatement.
	exitCaseStatement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#case_.
	enterCase_(ctx) {
	}

	// Exit a parse tree produced by oberonParser#case_.
	exitCase_(ctx) {
	}


	// Enter a parse tree produced by oberonParser#caseLabelList.
	enterCaseLabelList(ctx) {
	}

	// Exit a parse tree produced by oberonParser#caseLabelList.
	exitCaseLabelList(ctx) {
	}


	// Enter a parse tree produced by oberonParser#labelRange.
	enterLabelRange(ctx) {
	}

	// Exit a parse tree produced by oberonParser#labelRange.
	exitLabelRange(ctx) {
	}


	// Enter a parse tree produced by oberonParser#label.
	enterLabel(ctx) {
	}

	// Exit a parse tree produced by oberonParser#label.
	exitLabel(ctx) {
	}


	// Enter a parse tree produced by oberonParser#whileStatement.
	enterWhileStatement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#whileStatement.
	exitWhileStatement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#repeatStatement.
	enterRepeatStatement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#repeatStatement.
	exitRepeatStatement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#forStatement.
	enterForStatement(ctx) {
	}

	// Exit a parse tree produced by oberonParser#forStatement.
	exitForStatement(ctx) {
	}


	// Enter a parse tree produced by oberonParser#procedureDeclaration.
	enterProcedureDeclaration(ctx) {
	}

	// Exit a parse tree produced by oberonParser#procedureDeclaration.
	exitProcedureDeclaration(ctx) {
	}


	// Enter a parse tree produced by oberonParser#procedureHeading.
	enterProcedureHeading(ctx) {
	}

	// Exit a parse tree produced by oberonParser#procedureHeading.
	exitProcedureHeading(ctx) {
	}


	// Enter a parse tree produced by oberonParser#procedureBody.
	enterProcedureBody(ctx) {
	}

	// Exit a parse tree produced by oberonParser#procedureBody.
	exitProcedureBody(ctx) {
	}


	// Enter a parse tree produced by oberonParser#declarationSequence.
	enterDeclarationSequence(ctx) {
	}

	// Exit a parse tree produced by oberonParser#declarationSequence.
	exitDeclarationSequence(ctx) {
	}


	// Enter a parse tree produced by oberonParser#formalParameters.
	enterFormalParameters(ctx) {
	}

	// Exit a parse tree produced by oberonParser#formalParameters.
	exitFormalParameters(ctx) {
	}


	// Enter a parse tree produced by oberonParser#fPSection.
	enterFPSection(ctx) {
	}

	// Exit a parse tree produced by oberonParser#fPSection.
	exitFPSection(ctx) {
	}


	// Enter a parse tree produced by oberonParser#formalType.
	enterFormalType(ctx) {
	}

	// Exit a parse tree produced by oberonParser#formalType.
	exitFormalType(ctx) {
	}


	// Enter a parse tree produced by oberonParser#module.
	enterModule(ctx) {
	}

	// Exit a parse tree produced by oberonParser#module.
	exitModule(ctx) {
	}


	// Enter a parse tree produced by oberonParser#importList.
	enterImportList(ctx) {
	}

	// Exit a parse tree produced by oberonParser#importList.
	exitImportList(ctx) {
	}


	// Enter a parse tree produced by oberonParser#import_.
	enterImport_(ctx) {
	}

	// Exit a parse tree produced by oberonParser#import_.
	exitImport_(ctx) {
	}



}