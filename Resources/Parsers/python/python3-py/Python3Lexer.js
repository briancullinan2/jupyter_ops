"use strict";
// Generated from /Users/briancullinan/jupyter_ops/Resources/Parsers/python/python3-py/Python3.g4 by ANTLR 4.7.3-SNAPSHOT
...
var Python3Lexer = /** @class */ (function (_super) {
    ...
    __decorate([
        property
    ], Python3Lexer.prototype, "def", void 0);
    return Python3Lexer;
}(Lexer_1.Lexer));
exports.Python3Lexer = Python3Lexer;
return self._tokens;
except;
AttributeError: self._tokens = [];
return self._tokens;
def;
indents(self);
try { }
...
AttributeError: self._lastToken = None;
return self._lastToken;
def;
lastToken(self, value);
self._lastToken = value;
def;
reset(self);
(_this = _super.call(this) || this).reset();
self.tokens = [];
self.indents = [];
self.opened = 0;
self.lastToken = None;
def;
emitToken(self, t);
(_this = _super.call(this) || this).emitToken(t);
...
next = (_this = _super.call(this) || this).nextToken();
...
return dedent;
def;
commonToken(self, type, text, indent = 0);
...
if (text)
    ...
    stop;
return CommonToken(self._tokenFactorySourcePair, type, (_this = _super.call(this) || this).DEFAULT_TOKEN_CHANNEL, start, stop);
def;
...
def;
atStartOfInput(self);
return Lexer_1.Lexer.column.fget(self) == 0;
and;
Lexer_1.Lexer.line.fget(self) == 1;
...
void {
    ...
    spaces: spaces,
    la_char: la_char,
    try: la = self._input.LA(1),
    la_char: la_char, Python: Python, does: does, not: not, compare: compare, char: char, to: to, ints: ints, directly: directly,
    except: except, ValueError: , End: End, of: of, file: file,
    pass: pass,
    Strip: Strip, newlines: newlines, inside: inside, open: open, clauses: clauses, except: except, if: we, are: are, near: near, EOF: EOF, : .We, keep: keep, NEWLINEs: NEWLINEs, near: near, EOF: EOF, to: to,
    satisfy: satisfy, the: the, final: final, newline: newline, needed: needed, by: by, the: the, single_put: single_put, rule: rule, used: used, by: by, the: the, REPL: REPL, : .
        try,
    nextnext_la: nextnext_la,
    nextnext_la_char: nextnext_la_char,
    except: except, ValueError: nextnext_eof = True,
    else: nextnext_eof = False,
    if: self.opened > 0, or: or, nextnext_eof: nextnext_eof, is: is, False: False, and: function (la_char) { }
} == '\r';
or;
la_char == '\n';
or;
