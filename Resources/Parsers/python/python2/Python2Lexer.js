...
var CommonToken = ;
var IndentStack = /** @class */ (function () {
...
var Python2Lexer = /** @class */ (function (_super) {
    __extends(Python2Lexer, _super);
    function Python2Lexer() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    Object.defineProperty(Python2Lexer.prototype, "vocabulary", {
        // @Override
        // @NotNull
        get: function () {
    ...
    Python2Lexer.channelNames = [
        "DEFAULT_TOKEN_CHANNEL", "HIDDEN",
    ];
    // tslint:disable:no-trailing-whitespace
    Python2Lexer.modeNames = [
        "DEFAULT_MODE",
    ];
    Python2Lexer.ruleNames = [
    ...
    Python2Lexer.VOCABULARY = new VocabularyImpl_1.VocabularyImpl(Python2Lexer._LITERAL_NAMES, Python2Lexer._SYMBOLIC_NAMES, []);
    return Python2Lexer;
}(Lexer_1.Lexer));
...
t = (_this = _super.call(this, Python2Lexer, self) || this).nextToken();
if (t.type != Token.EOF)
    : return t;
else
    : if (not)
        self._suppressNewlines;
self.emitNewline();
self.emitFullDedent();
self.emitEndmarker();
self.emitEndToken(t);
return self._tokens.deq();
def;
emitEndToken(self, token);
self._tokens.enq(token);
def;
emitIndent(self, length = 0, text = 'INDENT');
t = self.createToken(module_1.default.INDENT, text, length);
self._tokens.enq(t);
def;
emitDedent(self);
t = self.createToken(module_1.default.DEDENT, 'DEDENT');
self._tokens.enq(t);
def;
emitFullDedent(self);
while (not)
...
def;
emitEndmarker(self);
t = self.createToken(module_1.default.ENDMARKER, 'ENDMARKER');
self._tokens.enq(t);
def;
emitNewline(self);
t = self.createToken(module_1.default.NEWLINE, 'NEWLINE');
self._tokens.enq(t);
def;
createToken(self, type_, text = "", length = 0);
start = self._tokenStartCharIndex;
stop = start + length;
t = CommonToken(self._tokenFactorySourcePair, type_, self.DEFAULT_TOKEN_CHANNEL, start, stop);
t.text = text;
return t;
constructor(input, CharStream);
{
    _this = _super.call(this, input) || this;
    this._interp = new LexerATNSimulator_1.LexerATNSimulator(Python2Lexer._ATN, this);
}
get;
grammarFileName();
string;
...
string[];
{
    return Python2Lexer.ruleNames;
}
get;
serializedATN();
string;
{
    return Python2Lexer._serializedATN;
}
get;
channelNames();
string[];
{
    return Python2Lexer.channelNames;
}
get;
modeNames();
string[];
{
    return Python2Lexer.modeNames;
}
...
LINENDING_action(_localctx, RuleContext_1.RuleContext, actionIndex, number);
void {
    switch: function (actionIndex) {
    },
    case: 0,
    self: self, : ._lineContinuation = False,
    break: ,
    case: 1,
    self: self, : ._lineContinuation = True,
    break: ,
    case: 2,
    if: self._openBRCount == 0, and: and, not: not, self: self, : ._lineContinuation,
    if: not, self: self, : ._suppressNewlines,
    self: self, : .emitNewline(),
    self: self, : ._suppressNewlines = True,
    la: la,
    if: la, not: not, in: [ord(' '), ord('\t'), ord('#')],
    self: self, : ._suppressNewlines = False,
    self: self, : .emitFullDedent(),
    break: 
};
WHITESPACE_action(_localctx, RuleContext_1.RuleContext, actionIndex, number);
void {
    switch: function (actionIndex) {
    },
    case: 3,
    if: function (self) { }, : ._tokenStartColumn == 0, and: and, self: self, : ._openBRCount == 0,
    and: and, not: not, self: self, : ._lineContinuation,
    la: la,
    if: la, not: not, in: [ord('\r'), ord('\n'), ord('#'), -1],
    self: self, : ._suppressNewlines = False,
    wsCount: wsCount,
    for: ch in self.text,
    if: ch == ' ', wsCount: wsCount, 1: elif, ch: ch
} == '\t';
wsCount += 8;
...
OPEN_BRACKET_action(_localctx, RuleContext_1.RuleContext, actionIndex, number);
void {
    switch: function (actionIndex) {
    },
    case: 8,
...
CLOSE_BRACKET_action(_localctx, RuleContext_1.RuleContext, actionIndex, number);
void {
    switch: function (actionIndex) {
    },
    case: 9,
...
_serializedATNSegment0: string =
    "\x03\uC91D\uCABA\u058D\uAFBA\u4F53\u0607\uEA8B\uC241\x02T\u02B3\b\x01" +
        ...
        "\f\f\x0F\x0F$$^^\x03\x02^^\x04\x02\f\f\x0F\x0F\x02\u02EE\x02\x03\x03\x02" +
        ...
        "\x02\u01FA\u01FB\x03\x02\x02\x02\u01FB\u0212\x03\x02\x02\x02\u01FC";
_serializedATNSegment1: string =
    "\u01FE\t\t\x02\x02\u01FD\u01FC\x03\x02\x02\x02\u01FE\u01FF\x03\x02\x02" +
        "\x02\u01FF\u01FD\x03\x02\x02\x02\u01FF\u0200\x03\x02\x02\x02\u0200\u020F" +
        "\x03\x02\x02\x02\u0201\u0210\t\x06\x02\x02\u0202\u0204\t\x07\x02\x02\u0203" +
        "\u0205\t\b\x02\x02\u0204\u0203\x03\x02\x02\x02\u0204\u0205\x03\x02\x02" +
        "\x02\u0205\u0207\x03\x02\x02\x02\u0206\u0208\t\t\x02\x02\u0207\u0206\x03" +
        "\x02\x02\x02\u0208\u0209\x03\x02\x02\x02\u0209\u0207\x03\x02\x02\x02\u0209" +
        "\u020A\x03\x02\x02\x02\u020A\u020C\x03\x02\x02\x02\u020B\u020D\t\x0E\x02" +
        "\x02\u020C\u020B\x03\x02\x02\x02\u020C\u020D\x03\x02\x02\x02\u020D\u0210" +
        "\x03\x02\x02\x02\u020E\u0210\t\x0E\x02\x02\u020F\u0201\x03\x02\x02\x02" +
        "\u020F\u0202\x03\x02\x02\x02\u020F\u020E\x03\x02\x02\x02\u020F\u0210\x03" +
        "\x02\x02\x02\u0210\u0212\x03\x02\x02\x02\u0211\u01B3\x03\x02\x02\x02\u0211" +
...
ATN_1.ATN;
{
    if (!Python2Lexer.__ATN) {
        Python2Lexer.__ATN = new ATNDeserializer_1.ATNDeserializer().deserialize(Utils.toCharArray(Python2Lexer._serializedATN));
    }
