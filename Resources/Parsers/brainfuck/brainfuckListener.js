// Generated from ./brainfuck/brainfuck.g4 by ANTLR 4.10.1
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by brainfuckParser.
export default class brainfuckListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by brainfuckParser#file_.
	enterFile_(ctx) {
	}

	// Exit a parse tree produced by brainfuckParser#file_.
	exitFile_(ctx) {
	}


	// Enter a parse tree produced by brainfuckParser#statement.
	enterStatement(ctx) {
	}

	// Exit a parse tree produced by brainfuckParser#statement.
	exitStatement(ctx) {
	}


	// Enter a parse tree produced by brainfuckParser#opcode.
	enterOpcode(ctx) {
	}

	// Exit a parse tree produced by brainfuckParser#opcode.
	exitOpcode(ctx) {
	}



}