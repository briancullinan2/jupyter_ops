...
vertex
    : 'v' x = decimal y = decimal z = decimal w = decimal?
    ;
...
vertex_texture
    : 'vt' u = decimal v = decimal? w = decimal?
    ;
...
faces
    : 'f' v = (INTEGER | INTEGER_PAIR | INTEGER_TRIPLET)+
    ;

// Block defining a free-form surface
free_form_surface
    : (curve | curve2d | surface) NL+ (
        (parameter | outer_trimming_loop | inner_trimming_loop | special_curve | special_point) NL+
    )* end
    ;
...
curve
    : 'curv' u0 = decimal u1 = decimal v = INTEGER+
    ;
...
// coordinate (weight) defaults to 1.0 if omitted.
surface
    : 'surf' s0 = decimal s1 = decimal t0 = decimal t1 = decimal v = (
        INTEGER
        | INTEGER_PAIR
        | INTEGER_TRIPLET
    )+
...
outer_trimming_loop
    : 'trim' (u0 = decimal u1 = decimal curv2d = INTEGER)+
    ;
...
//   defined with the curv2 statement.
inner_trimming_loop
    : 'hole' (u0 = decimal u1 = decimal curv2d = INTEGER)+
    ;
...
//   defined with the curv2 statement.
special_curve
    : 'scrv' (u0 = decimal u1 = decimal curv2d = INTEGER)+
    ;
...
//   must have been previously defined with the curv2 statement.
connectivity
    : 'con' surf_1 = INTEGER q0_1 = decimal q1_1 = decimal curv2d_1 = INTEGER surf_2 = INTEGER q0_2 = decimal q1_2 = decimal curv2d_2 = INTEGER
    ;
...
merging_group
    : 'mg' (group_number = INTEGER res = decimal | 'off')
    ;
