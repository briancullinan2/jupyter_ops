// Generated from C:/Users/megam/jupyter_ops/Resources/Parsers/metric/metric.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by metricParser.
export default class metricListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by metricParser#file_.
	enterFile_(ctx) {
	}

	// Exit a parse tree produced by metricParser#file_.
	exitFile_(ctx) {
	}


	// Enter a parse tree produced by metricParser#uom.
	enterUom(ctx) {
	}

	// Exit a parse tree produced by metricParser#uom.
	exitUom(ctx) {
	}


	// Enter a parse tree produced by metricParser#measure.
	enterMeasure(ctx) {
	}

	// Exit a parse tree produced by metricParser#measure.
	exitMeasure(ctx) {
	}


	// Enter a parse tree produced by metricParser#exponent.
	enterExponent(ctx) {
	}

	// Exit a parse tree produced by metricParser#exponent.
	exitExponent(ctx) {
	}


	// Enter a parse tree produced by metricParser#prefix_.
	enterPrefix_(ctx) {
	}

	// Exit a parse tree produced by metricParser#prefix_.
	exitPrefix_(ctx) {
	}


	// Enter a parse tree produced by metricParser#unit.
	enterUnit(ctx) {
	}

	// Exit a parse tree produced by metricParser#unit.
	exitUnit(ctx) {
	}


	// Enter a parse tree produced by metricParser#baseunit.
	enterBaseunit(ctx) {
	}

	// Exit a parse tree produced by metricParser#baseunit.
	exitBaseunit(ctx) {
	}


	// Enter a parse tree produced by metricParser#derivedunit.
	enterDerivedunit(ctx) {
	}

	// Exit a parse tree produced by metricParser#derivedunit.
	exitDerivedunit(ctx) {
	}



}