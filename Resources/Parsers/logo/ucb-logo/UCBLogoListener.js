// Generated from C:/Users/megam/jupyter_ops/Resources/Parsers/logo/ucb-logo/UCBLogo.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by UCBLogoParser.
export default class UCBLogoListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by UCBLogoParser#parse.
	enterParse(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#parse.
	exitParse(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedureDefInstruction.
	enterProcedureDefInstruction(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedureDefInstruction.
	exitProcedureDefInstruction(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#macroDefInstruction.
	enterMacroDefInstruction(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#macroDefInstruction.
	exitMacroDefInstruction(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedureCallExtraInputInstruction.
	enterProcedureCallExtraInputInstruction(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedureCallExtraInputInstruction.
	exitProcedureCallExtraInputInstruction(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedureCallInstruction.
	enterProcedureCallInstruction(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedureCallInstruction.
	exitProcedureCallInstruction(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedure_def.
	enterProcedure_def(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedure_def.
	exitProcedure_def(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#macro_def.
	enterMacro_def(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#macro_def.
	exitMacro_def(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#variables.
	enterVariables(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#variables.
	exitVariables(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#body_def.
	enterBody_def(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#body_def.
	exitBody_def(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#body_instruction.
	enterBody_instruction(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#body_instruction.
	exitBody_instruction(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedure_call_extra_input.
	enterProcedure_call_extra_input(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedure_call_extra_input.
	exitProcedure_call_extra_input(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedure_call.
	enterProcedure_call(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedure_call.
	exitProcedure_call(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#expressions.
	enterExpressions(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#expressions.
	exitExpressions(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#notEqualsExpressionExpression.
	enterNotEqualsExpressionExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#notEqualsExpressionExpression.
	exitNotEqualsExpressionExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#arrayExpression.
	enterArrayExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#arrayExpression.
	exitArrayExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#additionExpression.
	enterAdditionExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#additionExpression.
	exitAdditionExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#wordExpression.
	enterWordExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#wordExpression.
	exitWordExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#numberExpression.
	enterNumberExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#numberExpression.
	exitNumberExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#parensExpression.
	enterParensExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#parensExpression.
	exitParensExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#multiplyExpression.
	enterMultiplyExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#multiplyExpression.
	exitMultiplyExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#greaterThanExpression.
	enterGreaterThanExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#greaterThanExpression.
	exitGreaterThanExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#variableExpression.
	enterVariableExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#variableExpression.
	exitVariableExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#divideExpression.
	enterDivideExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#divideExpression.
	exitDivideExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#lessThanEqualsExpression.
	enterLessThanEqualsExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#lessThanEqualsExpression.
	exitLessThanEqualsExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#greaterThanEqualsExpression.
	enterGreaterThanEqualsExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#greaterThanEqualsExpression.
	exitGreaterThanEqualsExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#unaryMinusExpression.
	enterUnaryMinusExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#unaryMinusExpression.
	exitUnaryMinusExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#quotedWordExpression.
	enterQuotedWordExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#quotedWordExpression.
	exitQuotedWordExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#equalsExpression.
	enterEqualsExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#equalsExpression.
	exitEqualsExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#subtractionExpression.
	enterSubtractionExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#subtractionExpression.
	exitSubtractionExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedureCallExpression.
	enterProcedureCallExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedureCallExpression.
	exitProcedureCallExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#lessThanExpression.
	enterLessThanExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#lessThanExpression.
	exitLessThanExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#procedureCallExtraInput.
	enterProcedureCallExtraInput(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#procedureCallExtraInput.
	exitProcedureCallExtraInput(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#listExpression.
	enterListExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#listExpression.
	exitListExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#nameExpression.
	enterNameExpression(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#nameExpression.
	exitNameExpression(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#array_.
	enterArray_(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#array_.
	exitArray_(ctx) {
	}


	// Enter a parse tree produced by UCBLogoParser#list_.
	enterList_(ctx) {
	}

	// Exit a parse tree produced by UCBLogoParser#list_.
	exitList_(ctx) {
	}



}